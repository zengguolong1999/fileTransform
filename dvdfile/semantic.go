...
//TransToLinesMem does the same as TransToLines. However, it allocates new memory and the modification made to result lines is invisible to text.
func TransToLinesMem(text []byte) (lines [][]byte) {
    start, end := 0, 0
    lines = make([][]byte, 0)
    for i, v := range text {
        if v == eol {
            end = i+1
            newLine := make([]byte, end-start)
            copy(newLine, text[start:end])
...
//TransToParasMem does the same as TransToParas. However, allocate new memory, so you can modify each line in each paragraph without making overlap mistake.
func TransToParasMem(text []byte) (paras [][][]byte) {
    lines := TransToLinesMem(text)
    var emptyLine = []byte("\r\n")
    for i:=0; i<len(lines)-1; i++ { //don't consider lastline
        line := lines[i]
        if line[len(line)-2] != '\r' {
    ...
    for i:=0; i<len(lines); i++ {
        if sliceEq(lines[i], emptyLine) {
            ...
            newPara := make([][]byte, end-start)
            copy(newPara, lines[start:end])
            paras = append(paras, newPara)
            start = i+1
            i += 1 //Skip next "if" statement check, because it has been checked.
        }
    }
    if end != len(lines) { //for last line which has no empty line below it.
        newPara := make([][]byte, len(lines[end:]))
        copy(newPara, lines[end:])
        paras = append(paras, newPara)
    }
    return
}

//TransToParasNoLine transforms text into paras without line concept. i.e. paras[0] indicates the first paragraph. This function is provided as a easier way to access character from paragraph without access line previously.  Allocate no new memory and the modification made to lines is visible to text.
...
//TransToParasMemNoLine does the same as TransToParasNoLine. However, it allocates new memory and the modification made to paras is invisible to text.
func TransToParasMemNoLine(text []byte) (paras [][]byte) {
    paras3d := TransToParas(text)
    paras = make([][]byte, 0)
    for _, para := range paras3d {
        newPara := make([]byte, 0)
        for _, line := range para {
            newPara = append(newPara, line...)
        }
        paras = append(paras, newPara)
    }
    return
